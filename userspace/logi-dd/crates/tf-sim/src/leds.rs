...
impl RevLeds {
    ...
    /// first (`LOGI_DD_SYSFS_DIR`, when set, overrides that scan with a
    /// single device directory - the same development/test override the
    /// logi-dd front-ends honor, and additionally checked for a `leds`
    /// subdirectory shaped like a classdev quintet); otherwise the first
    /// classdev quintet found under `/sys/class/leds` (the G923's rev
    /// strip) is used.
