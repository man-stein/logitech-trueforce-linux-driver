...
fn sdk_status(field: &str, installer: Option<&std::path::Path>) -> (Option<std::path::PathBuf>, String) {
    match logi_wheel_core::steam::resolve_sdk_dir(field, installer) {
        Some(dir) => {
...
fn refresh_tf_daemon(app_weak: slint::Weak<App>, expect_running: Option<bool>) {
    std::thread::spawn(move || {
        let mut running = logi_wheel_core::tfsim::daemon_pid().is_some();
        if let Some(expected) = expect_running {
            for _ in 0..10 {
                ...
                running = logi_wheel_core::tfsim::daemon_pid().is_some();
            }
        }
        let _ = slint::invoke_from_event_loop(move || {
...
fn report_tf_write(app: &App, result: Result<(), logi_wheel_core::Error>) {
    if let Err(e) = result {
...
/// explicit not-loaded marker (`logi_wheel_core::driver` reads the sysfs
/// file; absent means no module).
fn driver_version_text() -> String {
    logi_wheel_core::driver::module_version()
        .unwrap_or_else(|| "(module not loaded)".to_string())
}

/// Dev-only render aid: `LOGI_WHEEL_TEST_OVERLAYS=<degrees>` forces every
...
/// unless the variable is set; read once. The pre-rename
/// `LOGI_DD_TEST_OVERLAYS` still works as a deprecated alias (the new name
/// wins if both are set); see [`overlay_debug_from`].
fn overlay_debug() -> Option<f32> {
    static FLAG: std::sync::OnceLock<Option<f32>> = std::sync::OnceLock::new();
    *FLAG.get_or_init(|| {
        overlay_debug_from(
            std::env::var("LOGI_WHEEL_TEST_OVERLAYS").ok(),
            std::env::var("LOGI_DD_TEST_OVERLAYS").ok(),
        )
    })
}

/// Pure parsing core of [`overlay_debug`], taking the two variables' raw
/// values directly so the new-name/old-alias/both-set precedence is testable
/// without touching the real environment or the `OnceLock` cache.
fn overlay_debug_from(new: Option<String>, old: Option<String>) -> Option<f32> {
    new.or(old).map(|v| v.trim().parse().unwrap_or(0.0))
}
...
fn start_test_monitor(
    ...
    std::thread::spawn(move || {
        ...
        // actually has.
        let (model, range) = match logi_wheel_core::Device::discover() {
            Ok(d) => {
...
fn main() -> Result<(), slint::PlatformError> {
    ...
    app.set_project_url(logi_wheel_core::PROJECT_URL.into());
    ...
    // `logi_wheel_core::helpers`. The status lines show the resolved path.
    let ffb_binary = logi_wheel_core::helpers::ffb_path();
    ...
    let installer_path: Option<std::path::PathBuf> = logi_wheel_core::helpers::installer_path();
    ...
    let tf_binary: Option<String> =
        logi_wheel_core::helpers::tf_sim_path().map(|p| p.to_string_lossy().into_owned());
    ...
    // key (logi_wheel_core::tfsim's writers preserve everything else in the
    ...
    }
    {
        ...
        app.on_setup_tf_set_intensity(move |v| {
            ...
            report_tf_write(
                &app,
                logi_wheel_core::tfsim::set_intensity_in(
    ...
    }
    {
        ...
        app.on_setup_sim_game_intensity(move |id, v| {
            ...
            report_tf_write(
                &app,
                logi_wheel_core::tfsim::set_game_intensity_in(
                    &logi_wheel_core::tfsim::default_path(),
                    &id,
                    v.clamp(0, 100) as u8,
                ),
    ...
    }
    {
        ...
        app.on_setup_tf_stop_daemon(move || {
            if let Some(pid) = logi_wheel_core::tfsim::daemon_pid() {
                send_sigterm(pid);
            }
...
mod overlay_debug_tests {
    ...
    #[test]
    fn new_only() {
        assert_eq!(overlay_debug_from(Some("12".to_string()), None), Some(12.0));
    }

    #[test]
    fn old_alias_only() {
        assert_eq!(overlay_debug_from(None, Some("34".to_string())), Some(34.0));
    }

    #[test]
    fn new_wins_when_both_set() {
        assert_eq!(overlay_debug_from(Some("1".to_string()), Some("2".to_string())), Some(1.0));
    }

    #[test]
    fn neither_set_is_inert() {
        assert_eq!(overlay_debug_from(None, None), None);
    }
