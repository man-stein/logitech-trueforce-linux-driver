...
/// grouping lives here rather than in `logi-wheel-core`'s registry so the TUI
...
pub fn curve_ghost_y_frac(curve: &Curve, x_frac: f32) -> f32 {
    let output = logi_wheel_core::curve::interp(&curve.compose(), from_frac(x_frac));
...
//
// `ui/rgb_strip.slint` never touches `logi_wheel_core::Color`: each swatch is
...
//
// `ui/slot_text.slint` never touches `logi_wheel_core::Value`: it renders one
...
/// across all launchers (`games`, from [`logi_wheel_core::launchers::discover`])
...
mod tests {
    ...
    use logi_wheel_core::{Category, Device};
    ...
    use logi_wheel_core::shaping::{Axis, AxisToggles};
    ...
    fn every_classic_registry_attr_has_a_group() {
        for s in logi_wheel_core::CLASSIC_REGISTRY {
            assert!(!attr_group(s.attr).is_empty(), "no group for {}", s.attr);
        }
    }
